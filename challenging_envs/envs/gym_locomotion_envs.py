...
class WalkerBaseBulletEnv(MJCFBaseBulletEnv):
  ...
  def load_obstacle(self, pos, ori, shape, height):
    if shape == 'cube':
      path = os.path.abspath('.')
      idx = self._p.loadURDF(os.path.join(path, 'challenging_envs/envs/urdfs/cube/cube.urdf'), pos, self._p.getQuaternionFromEuler(ori), useFixedBase=True)
      pos = (pos[0], pos[1], height)
      self._p.resetBasePositionAndOrientation(idx, pos, self._p.getQuaternionFromEuler(ori))
    else:
        assert False, "we do not support other shapes for now, please add it in challenging_envs/envs/gym_locomotion_envs.py"
    return idx

  def remove_obstacle(self, obs_idx):
