...
class BayesianSequenceModel(nn.Module):
    def __init__(self, state_size=21, action_size=8, z_size=32, hidden_state_size=256, likelihood_std=0.1, device=None, path=None):
        ...
        # self.iafs = [affine_autoregressive(z_size, hidden_dims=[64]) for _ in range(2)]
        # self.iafs_modules = nn.ModuleList(self.iafs)

        self.priors = {
            "z" : {
                "loc"   :   nn.Parameter(0.01*torch.ones((1, self.z_size)).to(device), requires_grad=False),
    ...
    def model(self, X, A, Y, batch_size):
        ...
        with pyro.plate('data', X_.shape[0], device=self.device) as ix:
            ...
            for t in range(X.shape[1]):
                ...
                #             )
                # sigma = pyro.sample('sigma_{}'.format(t),
                #                 dist.Uniform(0.1*torch.ones((Y_hat.shape[0], self.output_size)).to(self.device), torch.ones(Y_hat.shape[0], self.output_size).to(self.device))
                #                     .to_event(1),
                #             )
