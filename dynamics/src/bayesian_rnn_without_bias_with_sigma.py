...
class Encoder(nn.Module):
    ...
    def forward(self, h):
        ...
        sigma_loc = torch.exp(-1.0/torch.abs(sigma[:, :self.sigma_output_size]))
...
#         self.linear_y = nn.Linear(in_features=32, out_features=output_size*2)
#         # self.linear5 = nn.Linear(in_features=16, out_features=output_size)

#     def forward(self, x, z):
#         out = torch.cat([x, z], dim=-1).to(z.device)
...
#         y = self.linear_y(out)
#         y_loc = y[:, :self.output_size]
#         y_scale = torch.nn.functional.softplus(y[:, self.output_size:])
...
class BayesianSequenceModel(nn.Module):
    ...
    def guide_step(self, t, a, prev_z, prev_h, prev_c):
        ...
        #         dist.Uniform(0.01*torch.ones((z.shape[0], self.output_size)).to(self.device),0.1*torch.ones((z.shape[0], self.output_size)).to(self.device))
        #             .to_event(1) 
        #         )     
        sigma = pyro.sample("sigma_{}".format(t),
                dist.Normal(sigma_loc, 0.01)
                    .to_event(1) 
                )   
        return z, h, c
    ...
    def model(self, X, A, Y, batch_size):
        ...
        with pyro.plate('data', A.shape[0], device=self.device) as ix:
            ...
            Y_hat_loc, Y_hat_scale, Z = self.prior(X=batch_X, N=batch_X.shape[0], T=batch_A.shape[1])    
            O = []
            ...
            for t in range(A.shape[1]):
                if t == 0:
                    print(Y_hat_scale[0, t, 0], Y_hat_scale[0, t, 1], Y_hat_scale[0, t, 2])
                obs = pyro.sample('obs_{}'.format(t),
                                dist.Normal(loc=Y_hat_loc[:, t, :], scale=Y_hat_scale[:, t, :])
                                    .to_event(1),
        ...
        return Y_hat_loc.data.cpu().numpy(), Z.data.cpu().numpy(), O.data.cpu().numpy()

    def prior(self, X, N, T):
        Y_loc, Y_scale, Z = [], [], []
        for t in range(T):
            y_loc, y_scale, z = self.prior_step(x=X[:, 0, :], N=N, t=t)
            Y_loc.append(y_loc)
            Y_scale.append(y_scale)
            Z.append(z)
        return torch.stack(Y_loc).transpose(0,1), torch.stack(Y_scale).transpose(0,1), torch.stack(Z).transpose(0,1)
    ...
    def prior_step(self, x, N, t):
        ...
        y_loc, y_scale = self.networks["decoder"](x, z)
        sigma = pyro.sample('sigma_{}'.format(t),
                                        dist.Normal(self.priors["sigma"]["loc"].expand(N, self.output_size),
                                                    self.priors["sigma"]["scale"].expand(N, self.output_size))
                                            .to_event(1),
                                    )
        # y_scale = torch.exp(sigma)
        y_scale = sigma

        return y_loc, y_scale, z
    ...
    def loss(self, X, A, Y, batch_size):
        ...
        batch_Y_hat, batch_Y_hat_std, batch_Z = poutine.replay(self.prior, trace=trace)(X=X[self.guide_cache["ix"]], N=batch_size, T=X.shape[1])
        batch_Y = Y[self.guide_cache["ix"]]
        return torch.mean(torch.abs(batch_Y-batch_Y_hat)).item(), batch_Z.data.cpu().numpy(), batch_Y_hat.data.cpu().numpy(), batch_Y_hat_std.data.cpu().numpy(), batch_Y.data.cpu().numpy()
