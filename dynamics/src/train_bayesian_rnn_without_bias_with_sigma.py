...
from bayesian_rnn_without_bias_with_sigma import BayesianSequenceModel
...
class Trainer:
    ...
    def train(self, epochs, batch_size, train, val):
        ...
        for i in range(1, epochs+1):
            ...
            train_batch_mae_loss, train_batch_Z, train_batch_Y_hat, train_batch_Y_hat_std, train_batch_Y = self.vrnn.loss(X=X, A=A, Y=Y, batch_size=batch_size)
            val_batch_mae_loss, val_batch_Z, val_batch_Y_hat, val_batch_Y_hat_std, val_batch_Y = self.vrnn.loss(X=X_val, A=A_val, Y=Y_val, batch_size=batch_size)
            ...
            if i % 10 == 0:    
                # train curves            
                self.wandb.log({
                    "train-x-velocity"    :   get_velocity_curve_std(title="x-velocity", true=train_batch_Y[0, :, 0], pred=train_batch_Y_hat[0, :, 0], std=train_batch_Y_hat_std[0, :, 0]),
                    "train-y-velocity"    :   get_velocity_curve_std(title="y-velocity", true=train_batch_Y[0, :, 1], pred=train_batch_Y_hat[0, :, 1], std=train_batch_Y_hat_std[0, :, 1]),
                    "train-z-velocity"    :   get_velocity_curve_std(title="z-velocity", true=train_batch_Y[0, :, 2], pred=train_batch_Y_hat[0, :, 2], std=train_batch_Y_hat_std[0, :, 2])
                }, commit=False)
                self.wandb.log({
                ...
                self.wandb.log({
                    "val-x-velocity"    :   get_velocity_curve_std(title="x-velocity", true=val_batch_Y[0, :, 0], pred=val_batch_Y_hat[0, :, 0], std=val_batch_Y_hat_std[0, :, 0]),
                    "val-y-velocity"    :   get_velocity_curve_std(title="y-velocity", true=val_batch_Y[0, :, 1], pred=val_batch_Y_hat[0, :, 1], std=val_batch_Y_hat_std[0, :, 1]),
                    "val-z-velocity"    :   get_velocity_curve_std(title="z-velocity", true=val_batch_Y[0, :, 2], pred=val_batch_Y_hat[0, :, 2], std=val_batch_Y_hat_std[0, :, 2])
                }, commit=False)
                self.wandb.log({
...
if __name__ == "__main__":
    ...
    machine = "honshu"
    notes = "attempting {a0...aT} input as opposed to {(s0,a0)...(sT,aT)} input with learned std"
    name = "learned_likelihood_std+time_standardization"
    ...
    vrnn = BayesianSequenceModel(state_size=config["state_size"], 
                                 ...
                                 device=device,
                                 path=os.path.join(wandb.run.dir, machine+"_"+name+"_bayesian_rnn_model_without_bias_with_sigma.pth"))
